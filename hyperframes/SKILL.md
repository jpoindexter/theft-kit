---
name: hyperframes
description: Create video compositions, animations, title cards, overlays, captions, voiceovers, audio-reactive visuals, shader backgrounds, procedural WebGL canvas layers, and scene transitions in HyperFrames HTML. Use when asked to build any HTML-based video content, add captions or subtitles synced to audio, generate text-to-speech narration, create audio-reactive animation (beat sync, glow, pulse driven by music), add animated text highlighting (marker sweeps, hand-drawn circles, burst lines, scribble, sketchout), create GLSL/WebGL shader visuals, or add transitions between scenes (crossfades, wipes, reveals, shader transitions). Covers composition authoring, timing, media, and the full video production workflow. For CLI commands (init, lint, preview, render, transcribe, tts) see the hyperframes-cli skill.
---
...
The locked company brand source, when present, is
`/workspace/library/design/DESIGN.md`. Treat it as authoritative over
HyperFrames house palettes and generic style adjectives. Read it first, then
translate its brand tokens into CSS custom properties on the composition root
before writing scene styles.

Check in this order:

1. **`/workspace/library/design/DESIGN.md` exists?** → Read it. Use its exact colors, fonts/typefaces, spacing rhythm, component treatment, imagery rules, motion tone, and "What NOT to Do" constraints.
2. **DESIGN.md exists in the project?** → Read it. Use its exact colors, fonts/typefaces, motion rules, imagery guidance, and "What NOT to Do" constraints.
3. **visual-style.md exists?** → Read it. Apply its `style_prompt_full` and structured fields. (Note: `visual-style.md` is a project-specific file. `visual-styles.md` is the style library with 8 named presets — different files.)
4. **User named a style** (e.g., "Swiss Pulse", "dark and techy", "luxury brand")? → Read [visual-styles.md](./visual-styles.md) for the 8 named presets. Generate a minimal DESIGN.md with: `## Style Prompt` (one paragraph), `## Colors` (3-5 hex values with roles), `## Typography` (1-2 font families), `## Imagery` (logo/photo/illustration/texture direction when relevant), `## What NOT to Do` (3-5 anti-patterns).
5. **None of the above?** → Ask 3 questions before writing any HTML:
...
Every composition must trace its palette, typography, and imagery treatment
back to the locked company DESIGN.md, a project DESIGN.md, visual-style.md, or
explicit user direction. If you're reaching for `#333`, `#3b82f6`, or `Roboto`
without seeing them in the brand source — you skipped this step.
</HARD-GATE>
...
For GLSL/WebGL shader backgrounds, particles, raymarched objects, SDF ornaments,
or procedural visual layers, read the `shader-authoring` skill before writing
the composition. HyperFrames shaders must be deterministic and seekable:

- compile shaders synchronously during page load
- render once at time 0
- drive `iTime` from a GSAP timeline tween, not `requestAnimationFrame`
- avoid `Date.now()`, `performance.now()`, `Math.random()`, async setup, and
  infinite repeats
- expose `window.__hf = { duration, seek }` synchronously at top level; the
  screenshot-mode capture path polls this adapter and can time out even when
  `window.__timelines` exists
- keep `window.__timelines["<composition-id>"]` registered too, so the same
  composition works in the normal timeline-driven path
- use `@hyperframes/shader-transitions` for scene-to-scene texture transitions
  when available; use custom shader canvases for ambient/procedural layers

If render logs say `HeadlessExperimental.beginFrame unavailable` and the engine
falls back to screenshot mode, be suspicious of live WebGL canvas layers. They
can produce a `ready` MP4 where DOM overlays render but the GL canvas is blank.
For must-see procedural visuals such as cells, organelles, particles, diagrams,
or generated scientific motion, prefer a deterministic Canvas2D `frame(t)`
renderer unless WebGL is essential. If WebGL is essential, inspect actual MP4
frames for nonblank canvas pixels before calling the video done.
...
- [ ] Final result is visually correct, not just technically rendered

## Render Success vs Visual Success

`render_status=ready` only means the pipeline produced an MP4 successfully. It does **not** prove the composition is good.

Before calling the work done, sanity-check for these failure modes:

- Blank or near-blank frames where only the background renders
- Blank WebGL/procedural canvas layers where DOM overlays still render,
  especially after screenshot-mode fallback
- A single static frame for the whole video because the timeline never animates
- Scene content offscreen because CSS was written for the animated start state instead of the hero frame
- Text or stats rendered but clipped, overlapped, or unreadable
- Missing transitions, causing scenes to feel like jump cuts

If the video is blank or visually wrong, treat it as a composition bug even if the render succeeded. Fix the HTML/CSS/GSAP and re-run the `create_hyperframes_composition` tool.
...
- **[`shader-authoring`](../shader-authoring/SKILL.md)** — GLSL/WebGL technique routing, ShaderToy-to-WebGL2 adaptation, deterministic HyperFrames shader canvas pattern, and HTML/iframe artifact shader guidance. Read for shader backgrounds, ray marching, particles, procedural noise, SDF visuals, or custom WebGL layers.
...
## Hybrid cuts: HyperFrames + Canvas Video Studio

HyperFrames is the right surface for authoring deterministic motion graphics, title cards, end cards, and structured promo templates. It is NOT the right surface for stitching together pre-rendered MP4s from other generators (Seedance footage, agent-media talking-head).

For trailer-style or multi-beat social cuts that need both designed cards and realistic footage:

1. Author the title / end / interstitial cards here in HyperFrames (`create_hyperframes_composition`). Each card lands as its own video artifact + Library file.
2. Generate the realistic scene beats via Seedance (`openrouter-video-create`) or agent-media. Each render lands as its own Library file.
3. If the cut needs standalone narration, generate it via `create_voiceover_audio_artifact`.
4. Open Canvas Video Studio (`create_video_project_artifact`) referencing all of the rendered MP4s and audio files as `sources`. Edit the timeline to sequence + trim + add captions across the full cut.
5. Render `preview` first to verify the assembly, then `full` for the publish-ready output.

The assembly step belongs in Canvas Video Studio, not HyperFrames — HTML composition is the wrong abstraction for sequencing pre-rendered videos.

For purely designed video (no realism, no UGC), stay here in HyperFrames end-to-end.

For edit-only follow-ups on a finished HyperFrames render (reframe to 9:16, change captions, repurpose), hand off to Canvas Video Studio rather than re-rendering the composition with edited timing — see the `canvas-video-studio` skill.
