...
**Marker-style effects layer well on top of karaoke.** For emphasis words that need more than color/scale, add a marker effect — highlight sweep, circle, burst, or scribble. Match mode to energy: burst for hype, circle for key terms, highlight for standard, scribble for subtle.
...
Generate the audio data file ahead of time with any frequency-band analyzer (ffmpeg + a short Python script, or a dedicated audio analysis tool). The output should be JSON shaped like `{ fps, totalFrames, frames: [{ bands: [...] }] }` matching the example above.
...
**Marker effects layer well with karaoke** — use karaoke for the word-by-word reveal, then add a marker effect on emphasis words only. For example: karaoke highlights each word in white, but brand names get a yellow highlight sweep and stats get a red circle. Cycle marker modes across groups for visual variety.
