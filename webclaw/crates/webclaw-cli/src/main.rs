...
use webclaw_fetch::{
    AntibotConfig, BatchExtractResult, BrowserProfile, CrawlConfig, CrawlResult, Crawler,
...
struct Cli {
    ...
    /// Enable anti-bot mode: adaptive pacing + circuit breaker
    #[arg(long)]
    antibot: bool,

    /// Use headless browser for JS rendering. Auto-falls back when SPA detected.
    #[arg(long)]
    js: bool,
...
fn build_fetch_config(cli: &Cli) -> FetchConfig {
    ...
    let antibot = if cli.antibot {
        eprintln!("antibot: adaptive pacing + circuit breaker enabled");
        Some(AntibotConfig::default())
    } else {
        None
    };
...
async fn fetch_and_extract(cli: &Cli) -> Result<FetchOutput, String> {
    ...
    // --js: skip HTTP, go straight to browser rendering
    let options = build_extraction_options(cli);
    if cli.js {
        eprintln!("\x1b[36minfo:\x1b[0m browser rendering (--js)...");
        let browser_timeout = std::time::Duration::from_secs(cli.timeout.max(30));
        match webclaw_browser::render(url, browser_timeout).await {
            Ok(browser_result) => {
                eprintln!(
                    "\x1b[36minfo:\x1b[0m browser rendered {} ({} bytes)",
                    browser_result.url,
                    browser_result.html.len()
                );
                match webclaw_core::extract_with_options(
                    ...
                    Ok(extraction) if extraction.metadata.word_count > 10 => {
                        return Ok(FetchOutput::Local(Box::new(extraction)));
                    }
                    Ok(_) => {
                        eprintln!("\x1b[33mwarning:\x1b[0m browser rendered but extraction empty");
                    }
                    Err(e) => {
                        eprintln!("\x1b[33mwarning:\x1b[0m browser extraction failed: {e}");
                    }
                }
            }
            Err(e) => {
                eprintln!("\x1b[33mwarning:\x1b[0m browser rendering failed: {e}");
            }
        }
        // Fall through to HTTP if browser failed
    }

    // Normal path: HTTP fetch
    let client =
    ...
    if !matches!(reason, EmptyReason::None) {
        // Auto-fallback to browser for SPAs (when --js wasn't already tried)
        if matches!(reason, EmptyReason::JsRequired) {
            eprintln!("\x1b[36minfo:\x1b[0m falling back to browser rendering...");
            let browser_timeout = std::time::Duration::from_secs(cli.timeout.max(30));
            match webclaw_browser::render(url, browser_timeout).await {
                Ok(browser_result) => {
                    eprintln!(
                    ...
                    match webclaw_core::extract_with_options(
                        ...
                        Ok(extraction) if extraction.metadata.word_count > 50 => {
                            return Ok(FetchOutput::Local(Box::new(extraction)));
                        }
                        Ok(_) => {
                            eprintln!(
                                "\x1b[33mwarning:\x1b[0m browser rendered but extraction still empty"
                            );
                        }
                        Err(e) => {
                            eprintln!(
                                "\x1b[33mwarning:\x1b[0m browser extraction failed: {e}"
                            );
                        }
                    }
                }
        ...
        // Then try cloud API if available
