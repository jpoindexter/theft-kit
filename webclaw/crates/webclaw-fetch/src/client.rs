...
use webclaw_antibot::{AdaptivePacer, CircuitBreaker, PacerConfig, poisson_delay};
...
pub struct FetchConfig {
    ...
    /// Enable anti-bot pacing and circuit breaking. None = disabled (default).
    pub antibot: Option<AntibotConfig>,
}

/// Configuration for the anti-bot subsystem (adaptive pacing + circuit breaker).
///
/// When enabled, every outgoing request is gated: the circuit breaker rejects
/// requests after consecutive failures, and the adaptive pacer adds jittered
/// delays that speed up on success and slow down on failure/rate-limiting.
#[derive(Debug, Clone)]
...
impl FetchClient {
    ...
    pub fn new(config: FetchConfig) -> Result<Self, FetchError> {
        ...
        let (pacer, breaker) = match config.antibot {
            Some(ab) => {
                debug!("antibot enabled: pacer + circuit breaker");
                (
                    Some(AdaptivePacer::new(ab.pacer)),
                    Some(CircuitBreaker::new(ab.breaker_threshold, ab.breaker_cooldown)),
                )
            }
            None => (None, None),
        };

        Ok(Self { pool, pdf_mode, pacer, breaker })
    }
    ...
    pub async fn fetch(&self, url: &str) -> Result<FetchResult, FetchError> {
        ...
        for (attempt, delay) in delays.iter().enumerate() {
            // When antibot is off, use fixed retry delays. When on, the pacer
            // inside fetch_once() handles pacing, so skip the fixed sleep.
            if attempt > 0 && self.pacer.is_none() {
            ...
            match self.fetch_once(url).await {
                ...
                Err(FetchError::CircuitOpen(msg)) => {
                    // Circuit is open, don't retry -- it'll just fail again.
                    return Err(FetchError::CircuitOpen(msg));
                }
                Err(e) => {
    ...
    /// Single fetch attempt with antibot gating.
    async fn fetch_once(&self, url: &str) -> Result<FetchResult, FetchError> {
        ...
        if let Some(breaker) = &self.breaker {
            breaker.allow().map_err(FetchError::CircuitOpen)?;
        }

        // Adaptive pacing: jittered delay that adapts to server response
        ...
        let resp = match client.get(url).send().await {
            Ok(r) => r,
            Err(e) => {
                self.antibot_record_failure();
                return Err(e.into());
            }
        };
        let response = match Response::from_wreq(resp).await {
            Ok(r) => r,
            Err(e) => {
                self.antibot_record_failure();
                return Err(e);
        ...
        self.antibot_record_response(response.status(), elapsed, response.headers());
    ...
    pub async fn fetch_and_extract_with_options(
        ...
        // Antibot gate before the main request
        if let Some(breaker) = &self.breaker {
            breaker.allow().map_err(FetchError::CircuitOpen)?;
        }
        if let Some(pacer) = &self.pacer {
        ...
        self.antibot_record_response(response.status(), start.elapsed(), response.headers());

        // Cookie warmup: if we get a challenge page, visit the homepage first
        // to collect Akamai cookies (_abck, bm_sz, etc.), then retry.
        // Uses the same client so cookies persist across requests.
            ...
            && let Some(homepage) = extract_homepage(url)
        {
            ...
            // Pace the warmup requests too
            if let Some(pacer) = &self.pacer {
                let delay = pacer.current_delay();
                tokio::time::sleep(poisson_delay(delay, Duration::from_millis(50))).await;
            }
            ...
            self.antibot_record_response(response.status(), retry_start.elapsed(), response.headers());
    ...
    /// Record a successful or failed HTTP response on antibot subsystems.
    fn antibot_record_response(&self, status: u16, elapsed: Duration, headers: &http::HeaderMap) {
        if (200..400).contains(&status) {
            if let Some(pacer) = &self.pacer {
                pacer.record_success();
                pacer.record_latency(elapsed);
            }
            if let Some(breaker) = &self.breaker {
            ...
            if status == 429 {
                if let Some(pacer) = &self.pacer {
                    if let Some(ra) = headers.get("retry-after") {
                        if let Ok(secs) = ra.to_str().unwrap_or("").parse::<u64>() {
                            pacer.set_min_delay(Duration::from_secs(secs));
                        }
                    }
                }
            }
    ...
    /// Record a network-level failure (no response received).
    fn antibot_record_failure(&self) {
        if let Some(pacer) = &self.pacer {
    ...
    /// Returns current antibot stats for logging/debugging.
    pub fn antibot_stats(&self) -> Option<(f64, Duration, u32)> {
        self.pacer.as_ref().map(|p| {
            let trips = self.breaker.as_ref().map(|b| b.trips()).unwrap_or(0);
            (p.success_rate(), p.current_delay(), trips)
        })
    }
