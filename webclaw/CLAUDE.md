...
webclaw/
  crates/
    ...
    webclaw-fetch/    # HTTP client via wreq. Crawler. Sitemap discovery. Batch ops.
                      # + proxy pool rotation (per-request)
                      # + PDF content-type detection
                      # + document parsing (DOCX, XLSX, CSV)
    webclaw-antibot/  # Adaptive pacer + circuit breaker for anti-bot evasion
    webclaw-browser/  # Playwright sidecar for JS rendering (SPAs, anti-bot bypass)
    ...
    webclaw-api/      # REST API server (axum) — POST /v1/scrape
    webclaw-cli/      # CLI binary
  sidecar/            # Node.js Playwright sidecar script (render.mjs)
```

Three binaries: `webclaw` (CLI), `webclaw-mcp` (MCP server), `webclaw-api` (REST API).
